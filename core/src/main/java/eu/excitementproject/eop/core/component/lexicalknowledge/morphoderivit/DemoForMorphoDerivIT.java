...
public class DemoForMorphoDerivIT
{
public static void main(String[] args)
{
try
{
String morphoderivitFileName = args[0];
DemoForMorphoDerivIT app = new DemoForMorphoDerivIT(morphoderivitFileName);
app.go();
}
catch(Exception e)
{
...
public DemoForMorphoDerivIT(String morphoderivitFileName)
{
morphoderivitFile = new File(morphoderivitFileName);
}

public void go() throws LexicalResourceException, IOException, UnsupportedPosTagStringException
{
MorphoderivitLexicalResource morphoderivitResource = new MorphoderivitLexicalResource(morphoderivitFile);
...
processTwoLines(line.substring(1),line2,morphoderivitResource);
}
else
{
...
List<LexicalRule<? extends EmptyRuleInfo>> rules = morphoderivitResource.getRulesForLeft(word, posObj);
displayRules(rules);
}
...
private void processTwoLines(String line, String line2, MorphoderivitLexicalResource morphoderivitResource) throws UnsupportedPosTagStringException, LexicalResourceException
{
String[] wap = line.split("/");
String word = wap[0];
String pos = wap[1];
...
List<LexicalRule<? extends EmptyRuleInfo>> rules = morphoderivitResource.getRules(word, posObj, word2, posObj2);
displayRules(rules);
}

private File morphoderivitFile;
}
